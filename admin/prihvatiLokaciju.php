<?php
    ...
    if(isset($_POST["prihvatiID"])){
        $sql = "Select id from korisnik where kor_ime = '$user[korisnik]' ";
        $userID = mysqli_fetch_array($db->selectDB($sql))[0];
        $prihvatiID = $_POST["prihvatiID"];
        $sad = Vrijeme()->format('Y-m-d H:i:s');
        $sql = "update lokacija_za_ronjenje set odobreno = 1 
                where id = $prihvatiID";

        $db->updateDB($sql);
        spremiLog($userID, "Admin action - Odobri lokaciju $prihvatiID",2);
        echo 'prihvaceno';
    }
    $db->zatvoriDB();
